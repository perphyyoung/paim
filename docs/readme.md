...
| [开发经验.md](./开发经验.md) | 值得借鉴的开发经验（重在「为什么」，代码细节从简），开头有经验索引表：缩略图懒自愈、打开目录或文件的推荐方式（Shell API）、全屏查看的体验优化（独立窗口）、相似度检索（外部 embedding 服务：向量存储与失效、跨模态、测试替身）。 |
...
| [lessons.md](./lessons.md) | 排查记录（Lessons Learned）：弹窗残留与初始化、资源管理器定位（`/select,` 竞态与 SHOpenFolderAndSelectItems）、全屏查看器独立窗口（原主窗口全屏往返过渡帧）与键位双监听/叠加层导航穿透（一次按键跳两格）、e2e 收尾与进程退出串行（用例全绿却整轮 exit 1）、打标签后特殊标签计数漏刷（内存聚合值 + 多写入口）、嵌套详情的「槽位」模型（原始详情不动、图像/提示词各最多一个）、tauri-specta 集成返工等，含根因与通用约束。 |
