...
| [lessons.md](./lessons.md) | 排查记录（Lessons Learned）：弹窗残留与初始化、资源管理器定位（`/select,` 竞态与 SHOpenFolderAndSelectItems）、全屏查看器独立窗口（原主窗口全屏往返过渡帧）与键位双监听/叠加层导航穿透（一次按键跳两格）、e2e 收尾与进程退出串行（用例全绿却整轮 exit 1）、打标签后特殊标签计数漏刷（内存聚合值 + 多写入口）、tauri-specta 集成返工等，含根因与通用约束。 |
