...
| 小节 | 内容 |
| --- | --- |
| [一、铁律](#一铁律) | 蓝/红/绿三色的硬性禁令：蓝只做正向确认，红只做破坏性，绿/琥珀只表状态 |
| [二、颜色](#二颜色) | 「动作-颜色」映射表；按钮动作色、状态与展示色一览 |
| [三、标签（TagChip）](#三标签tagchip) | TagChip 组件变体、计数徽章、尺寸与场景映射 |
...
| 标签管理页                  | `count` + `dimOnHover`，hover 时左右上角各一个镜像角钮（编辑=铅笔 svg 在左上，删除=`✕` 在右上），由外层容器提供                                        |
