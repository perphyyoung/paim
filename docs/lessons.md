...
| 26 | 三级链路第三跳「按钮可用却点了没反应」 | 目标**已在另一类槽里**时只重新赋值同一个 id：`:key` 不变 → 不重建，叠放顺序也不变 → 仍被上方的槽压着（表现成没反应）。修法：「已在槽里 → 抬到最上」+ 两档显式遮罩层级（50/51，给 InlineDialog 60 / 右键菜单 70 留余量）。 |
| 27 | 一批两张图「只关联上一张」（e2e 偶发） | e2e mock 图只用 `Date.now()` 播种 → 同毫秒两次调用写出**相同字节**，被应用的 MD5 去重（同内容复用一条记录）→ 第二张图根本没入库。修法：种子改成「序号 + 时间 + 随机」+ 连续重复直接抛错；用例改为断言「无错误 + 两条不同 id + 都非 duplicate」。 |
...
- **别用「条件隐藏入口」当护栏**：它会把 bug 伪装成「没这功能」。限制要做在**跳转目标**上；「置灰 + `title`」只能当临时手段，且一旦按槽位模型放开（本轮把「禁止二级跳转」的置灰去掉，见 `e2e/14`）就要连 title 一起收拾干净。
...
## 26. 三级链路第三跳「按钮可用却点了没反应」：目标已在槽里，但没被抬到最上

### 现象与根因

图像详情 → 提示词详情 → 图像详情（反向亦然：提示词 → 图像 → 提示词）走到第三层后，再点「编辑提示词」/「查看图像详情」：
按钮**没有置灰**，点了却**没有任何反应**。（阶段：二级跳转刚放开、护栏从入口拿掉之后。）

- 第三跳的目标**常常已经在另一类槽里**：第三层是上一层内容的跨类详情，很容易指回上一层已展示的那条；
- `openNested` 那时只是**重新赋值同一个 id**：槽的 `:key` 跟内容 id 走 → key 不变 → **不重建**；
- 而两个槽都固定 `z-50`，靠挂载顺序定上下（后挂载的在上）→ 赋值不改顺序 → 目标槽仍被上方的槽压着；
- 于是「槽里确实有它」在视觉与交互上都等于「什么都没发生」——「已在槽里」这个分支被当成了 no-op，
  而用户的意图是「把那个详情拿上来」。

### 修法与验证

- `openNested` 增「已在槽里」分支：**只置顶、不重复拉取**（重复赋值会白换 props，还可能触发宿主详情的数据重拉）；
- 栈里加 `front`（当前在最上的槽），并把两档遮罩层级作为 `imageZ` / `promptZ`（50 在下 / 51 在上）暴露，
  由 `<NestedDetailSlots>` 传给详情弹窗的 `z`；替换槽内容（`replaceNestedImage`）同样抬升；关闭时若关的正是最上那层则清空 `front`；
- 层级只取 50 / 51：必须低于 `InlineDialog` 的 `z-[60]` 与右键菜单 `z-[70]`，否则弹窗自己的子对话框会被自己的遮罩压住；
- 回归用例 `e2e/14` 后两条（三级链路两个方向）：断言「目标槽能直接点到关闭钮」——被压住时遮罩会拦住点击
  （Playwright 报 `intercepts pointer events`），**修复前必红**。

### 后续参考 / 通用约束

- **「已在某处打开」不是 no-op**：用户动作的意图是「看到它」；实现要么置顶、要么聚焦，静默返回就是 bug
  （浏览器 / 窗口管理器的既有语义：点到已开的窗口 = 把它提到最前）。
- 同类弹窗多实例叠放时 **DOM 顺序即层级**，而换内容 / 重建实例都会改挂载顺序 → 层级必须**显式**（z 或重新挂载），
  不能依赖「谁后挂载」。
- 叠放层级要给**子对话框**留余量（本项目 50/51 → 60 → 70 三档）；越权的 z 会把弹窗自己的弹窗压住。
- 同类教训：§24（护栏做在入口上 → 功能被隐藏）、§25（换槽不换实例 → 内容停在旧 id）。

## 27. 「两张图只关联上一张」：e2e mock 图同毫秒写出相同内容，被应用 MD5 去重

### 现象与根因

全量跑时 `e2e/01`「两张图像附带同一提示词，应只创建一个提示词并关联两张图像」偶发失败：
`两张图像都应关联到该提示词` 期望 2、收到 1；单跑往往又能过（载荷一变，两次 `writePng` 就跨过了毫秒边界）。

- `writePng` 用 `Date.now() % 0xffffff` 当像素种子 —— 精度只到毫秒，**同一毫秒内的两次调用写出完全相同的字节**；
- 应用（行为正确，非 bug）按 MD5 去重：`image_service::import_with` 命中 `find_by_md5` 即复用已存在记录并返回
  `is_duplicate = true`（不落新文件、不插新行）；
- 于是 `paths: [p1, p2]` 这一批只剩**一条**记录，第二张图的「关联提示词」无处可落 →
  断言表现成「少一张」，而不是一眼能看出根因的报错（`import_images` 把关联失败放进 `errors` 返回，
  用例当时没有检查它）；
- 量化：把同一段播种逻辑在紧循环里跑 2000 次，旧播种**连续两次内容相同 1917 次**（≈96%），新播种 0 次；
  真实用例里两次写入之间还有 `deflateSync` / `mkdirSync` / `writeFileSync` 的开销，命中率低得多但**非零**
  —— 这正是它表现为「偶发」而不是「必现」的原因。

`paim.log` 的迷惑点：`[PagedBlocks:image] total 更新 0 → 2` 看着像「两张都入库了」，其实是**同一实例**里
上一个用例（`01` 第 1 条）留下的那张 + 本批的 1 张（两用例共用同一实例，日志前缀同为 `[E2E w0-0]`）。

### 修法与验证

- `writePng` 种子改为「**自增序号 + 时间 + 随机**」三重混合 → 同进程内每次写入内容必不同；
- 加兜底自检：连续两次写出相同内容**直接抛错**（把「少一张」这种难查的断言失败换成指名道姓的根因）；
- `e2e/01` 该用例增断言：`import_images` 的 `errors` 为空、`results` 为两条**不同** id、两条都非 `is_duplicate`
  —— 一旦发生去重，报错会直接点出「内容重复」，不必再从关联数反推；
- 回归：`pnpm e2e 01-upload 03-replace 12-navigate 14-second` → **12 passed**（`03` 的两个替换分支依赖
  「覆写后 md5 必不同」，同样受益）。

### 后续参考 / 通用约束

- **造数 helper 的「随机」要真随机**：只依赖毫秒时间戳，等于把「两次调用是否同值」交给机器速度 —— 快机器上必现。
  要唯一就用**序号/计数**这类与时间无关的单调量（或 `randomUUID`），时间/随机只当补充。
- **被测系统有去重 / 幂等语义时，造数与断言要与之一致**：多份数据要保证「不同源」，单份数据要保证「同源」
  （`e2e/03` 的 SameImage 分支刻意**不**覆写 mock 文件，走的就是「同 md5」）。
- **失败信息要指向根因**：只断言「数量少了」几乎无法反推是去重；主动断言「无错误 + 记录数 + 非重复标记」，
  下一次失败才能自解释。
