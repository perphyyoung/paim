...
| 24 | 嵌套详情里右键没反应 / 点结果无反应或越点越多 | 嵌套详情的**槽位模型**：原始详情永不动，嵌套的图像 / 提示词各最多一个——同类结果换那一类的槽、跨类结果用本层自己的槽；共享的「结果跳转」要在每一层接线一次，漏一层就是静默无效。 |
...
## 24. 嵌套详情的「槽位」模型：原始详情不动，嵌套的图像 / 提示词各最多一个

### 现象与根因

图像详情 → 相似结果页 → 点提示词结果 → 打开的提示词详情是**嵌套态**（父级传了 `is-nested`），而它的内容右键菜单项写着
`v-if="similarityEnabled && !isNested"` → 右键弹出的是**空盒子**（看起来「没有右键菜单」）。
同一处嵌套层还**缺 `@open-*` 接线**：即使把菜单放开，「点同类结果」会静默无反应、「点跨类结果」会再叠一层（无上限）。
...
规则（对任何一层都成立）：**同类结果 → 换掉那一类已有的那个槽**（顶层详情由页面换，保持既有语义）；
**跨类结果 → 用本层自己的跨类槽**（已有则替换，不新开层）。⇒ 层数恒 ≤ 3、第 0 层永不被替换。
已知语义：替换**外层**槽会连带关掉它里面那层 —— 层级立刻收敛，不会「越点越多」。

### 修复（4 处，全是接线/条件）

- `PromptDetailModal` 内容右键菜单项：`v-if` 去掉 `!isNested`（保留 `similarityEnabled`）；
- `PromptDetailModal` 里嵌套的 `ImageDetailModal`：`@open-image="onOpenSimilarImage"`（同类 → 换本层嵌套图像）
  + `@open-prompt="onNestedOpenPrompt"`（跨类 → 关掉嵌套图像并上抛，宿主替换它的嵌套提示词槽）；
- 新增 `PromptDetailModal.onNestedOpenPrompt(id)`：关 `imgDetailOpen` + `emit("open-prompt", id)`（顶层由页面换详情）；
- `ImageDetailModal` 里嵌套的 `PromptDetailModal`：`@open-prompt="onOpenSimilarPrompt"`（同类 → 换本层嵌套提示词）。

### 后续（未做）

嵌套态下「查看图像详情」/「编辑提示词」按钮仍是禁用 + 「禁止二级跳转」提示。按本模型它们也能安全放开
（都落在「各自一个槽」里）——即所谓**放开二级跳转**，属另一处交互决策。

### 后续参考 / 通用约束

- 「详情里再开详情」这类嵌套，**层数必须由数据模型显式给上限**（这里是「每类一个槽」）；只补一半接线，表现就是「点了没反应」或「越点越多」，都很难从 UI 上看出来。
- 与 §22 同源：**共享组件的能力（这里是「结果跳转」）要在每一层各接线一次**，漏一层的表现往往是静默无效。
- 这类交互先用 e2e 钉住再改：`e2e/11` 的新用例覆盖「嵌套层菜单可见 → 跨类结果落本层槽（底部不动）→ 同类只替换不叠加 → 逐层关闭后底部原样」，修复前必红。
