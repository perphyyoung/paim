...
| 22 | 全屏查看按一次 → 跳两格 | 共用胶囊 `NavAndIndex` 已内置 document 键盘导航，查看器又自注册了一份 window 监听，一次按键走两次（≥3 张图才可见）；**能力已内置就别在调用方再实现一份**。 |
...
- 这类问题随**列表长度**开关：写用例要凑到能触发的规模（本例 ≥3），否则「通过」是假的。
- 排查时优先「两个猜想各写一条断言」，先让用例把 payload 侧与交互侧分开证伪，再动代码。
