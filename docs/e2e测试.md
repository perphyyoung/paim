...
| `PAIM_EMBEDDING_MOCK`       | debug 构建 | 相似度改用假 embedding（`MockEmbedder`）：按输入派生确定性伪向量（同输入同向量；任意两条只有 ≈0.1 的各向异性基线），索引与检索不再依赖真实 llama.cpp 服务。fixture 固定注入，阈值判定用例见 `e2e/11` |
...
| 11 | 相似度检索：两个详情右键入口 → 合并结果页（左相似图像 / 右相似提示词）；两栏条数 / 阈值 / 重置 / 重查互相独立；源文案规则；跨模态点击跳另一类详情；嵌套详情内继续搜索与「槽位」收敛（底部详情不动、各层最多一个、**换槽后内容确实换新**） | 图像详情 + 提示词详情 |
| 12 | 全屏与详情导航：双击第 N 张以第 N 张开场（索引不错位）；← / → 每次只走一格、末项不越界；叠加层（嵌套详情）打开时底层不被 ← / → 穿透 | 提示词详情（从外界导入多图）→ 查看器窗口 / 两个详情弹窗 |
| 13 | 特殊标签计数刷新：批量添加（提示词主页）/ 拖拽添加（图像主页）后「无标」chip 应消失；两个主页卡片单张收藏后「收藏」chip 应出现 / 消失 | 提示词主页 + 图像主页 |
| 14 | 二级跳转与三级链路：嵌套图像详情可「编辑提示词」、嵌套提示词详情可「查看图像详情」（嵌套态入口可用）；目标只落进对应槽位、不新增层，换槽后内容换新，底部原始详情不动；三级链路的第三跳若目标**已在槽里**则必须把它置顶（否则按钮可用却点了没反应） | 提示词详情 + 图像详情 |
