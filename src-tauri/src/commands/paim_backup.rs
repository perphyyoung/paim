...
use crate::domain::backup_common::{
    BackupExportSummary, BackupImportSummary, BackupInfo, BackupProgress,
};
...
#[tauri::command]
#[specta::specta]
pub async fn inspect_paim_backup(zip_path: String) -> Result<BackupInfo, AppError> {
...
pub async fn import_paim_backup(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        let bk = app.state::<BkDb>();
        paim_backup_service::import(&app, &bk, &zip_path, |p: BackupProgress| {
            let _ = p.emit(&app);
        })
    })
    .await
