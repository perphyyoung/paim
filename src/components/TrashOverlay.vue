...
<template>
  <Teleport to="body">
    <div v-if="open" class="fixed inset-0 z-50 flex flex-col bg-gray-100 dark:bg-gray-900">
      <!-- 头部：标题 + 批量操作 + 关闭 -->
      <div class="flex shrink-0 items-center justify-between px-6 pt-4">
        ...
        <div class="flex items-center gap-2">
          ...
          <button
            type="button"
            title="关闭"
            class="flex h-9 w-9 items-center justify-center rounded-lg text-gray-500 transition-colors hover:bg-gray-200 dark:text-gray-400 dark:hover:bg-gray-700"
            @click="emit('close')"
          >
      ...
      <!-- 卡片网格 -->
      <div v-else class="flex min-h-0 flex-1 gap-1 px-6">
        <VirtualGrid
          ref="gridRef"
