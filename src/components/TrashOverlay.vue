...
<template>
  <Teleport to="body">
    ...
    >
      <div
        class="flex h-[85vh] w-[90vw] max-h-[calc(100vh-80px)] max-w-[calc(100vw-80px)] flex-col overflow-hidden rounded-lg border border-gray-700 bg-gray-900"
      >
        <!-- 头部：四元素水平分布，间距大致相等 -->
        <div class="flex shrink-0 items-center justify-between gap-2 px-6 pt-4 pb-2">
          ...
          <button
            type="button"
            title="关闭"
            class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg transition-colors text-gray-400 hover:bg-gray-700"
            @click="emit('close')"
          >
            <svg
              viewBox="0 0 24 24"
        ...
        <div
          v-if="items.length === 0"
          class="flex flex-1 flex-col items-center justify-center gap-3 text-gray-500"
        >
          <svg
            viewBox="0 0 24 24"
            fill="none"
