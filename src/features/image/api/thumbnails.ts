...
import type { EnsureFixed, EnsureResult, RebuildProgress, RebuildSummary } from "@/bindings";

/** 全量重建结果摘要（rebuild_thumbnails 返回；success 含已存在跳过与新生成两类） */
export type ThumbnailRebuildSummary = RebuildSummary;

/** 重建进度推送（事件 thumbnail-rebuild-progress） */
export type ThumbnailRebuildProgress = RebuildProgress;
...
/** 懒自愈单条修复结果 */
export type ThumbnailEnsureFixed = EnsureFixed;

/** 懒自愈结果（ensure_image_thumbnails 返回） */
export type ThumbnailEnsureResult = EnsureResult;
