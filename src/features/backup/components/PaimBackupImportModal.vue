...
// 导入 paim 备份的进度/结果弹窗：open 时开始导入，监听后端进度事件，完成或失败后展示摘要。
...
watch(
  ...
  async (open) => {
    ...
    try {
      summary.value = await importPaimBackup(props.zipPath);
      phase.value = "done";
      emit("imported");
    } catch (e) {
  ...
  <Teleport to="body">
    ...
    >
      <div class="w-96 max-w-[90vw] rounded-lg border p-5 shadow-sm border-gray-700 bg-gray-800">
        <h3 class="text-center text-base font-semibold text-gray-100">导入 paim 备份</h3>
