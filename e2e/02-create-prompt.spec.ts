...
import { connectAppPage } from "./helpers";

test("新建提示词后，新卡片应置顶显示", async () => {
  const { browser, page } = await connectAppPage();

  // 回到提示词主页（01 用例结束后停留在图像页）
  const promptsNav = page.getByRole("link", { name: "提示词" });
  await expect(promptsNav).toBeVisible();
