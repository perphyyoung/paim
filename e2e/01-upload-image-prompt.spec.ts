...
import { connectAppPage, writePng } from "./helpers";

/// 配置注入的 mock 图像路径（select_images 测试缝返回值）
const mockImagePath = JSON.parse(process.env.PAIM_E2E_MOCK_IMAGE_PATHS ?? "[]")[0] as string;

test("上传图像附带提示词后，图像卡片应关联该提示词", async () => {
  const { browser, page } = await connectAppPage();
  // 应用侧与测试侧日志都打到输出，便于失败时定位卡在哪一步
  ...
  await expect(page.getByText(path.basename(mockImagePath))).toBeVisible();
