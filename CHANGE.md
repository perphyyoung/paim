...
### 新增：相似度检索（图搜图 / 文搜文 / 跨模态合并结果页）

- 向量来自外部本地 embedding 服务（llama.cpp `--embeddings --pooling last`），图像与提示词分别存 `images.vec` / `prompts.vec`（f32 LE、写入前 L2 归一化，点积即余弦），**不建新表、不存模型指纹**：换模型或改预处理规则后在设置页点「全量重建」即可。
- 设置页新增「相似度」分组：服务地址 + 连通性测试 + 索引并发（两类共用）；「图像相似度」「提示词相似度」各一个开关与一个索引面板（增量索引 / 全量重建 / 清空 + 进度与上次摘要），两条索引互不影响（各自的重入保护与进度事件）。
- 图像入库前统一预处理为「JPEG + 长边 1024」：服务端图像解码器不认 webp，直喂会被静默丢成「假图」（向量与画面无关）。提示词只算 `content` 的向量；保存时内容变化会把该行 `vec` 置 NULL，交给「增量索引」补算。
- 详情弹窗新增右键入口「搜索相似的图像和提示词」（图像详情右键图像区、提示词详情非编辑态右键内容），打开**合并结果页**：首行是查询源（提示词源显示内容；图像源有关联提示词时优先显示提示词内容，否则显示文件名），下面左栏「相似图像」、右栏「相似提示词」，两栏的条数 / 阈值 / 重置 / 重查与 loading、空、错态互相独立；结果卡片与主页同形（背景图 + 相似度 + 名称，提示词另有内容摘要），点击结果同侧跳该详情、跨模态叠加打开另一类详情。
- 后端检索命令 `similar_images` / `similar_prompts` 都接受 `source = Image | Prompt`：两条查询共用同一个查询向量（该模型是联合空间，跨模态可直接比余弦），只排除源自身那一侧。
- 修复：e2e 假 embedding（`MockEmbedder`）此前的取值区间是 `[-0.5, 0)`（均值 -0.25），任意两条伪向量的余弦恒为 ~0.75，会冒充「相似」让阈值判定类用例失真；改为零均值噪声 + 0.1 的各向异性基线（任意两条 ≈0.1，恒正且远低于默认阈值 0.5），并加断言锁住该性质。

验证：`cargo test`、`pnpm check`、`pnpm test:ui`、`pnpm e2e`（29 项通过，含新增 `e2e/11` 覆盖两个入口、两栏独立重查与跨模态跳转）。

## v0.2.19

### 新增：添加标签入口支持标签自动完成（对齐 pm）
...
- 标签用例从 `image_service.test.rs` / `prompt_service.test.rs` 迁至新建 `domain/tag_service.test.rs`（拒绝缺失实体、批量整批回滚、updated_at 刷新），并新增 count / tags_map 忽略回收站条目的用例。
